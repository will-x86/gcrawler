...
type TursoQueueOptions struct {
	DBPath     string
	MaxRetries int
}

func NewTursoQueue(opts TursoQueueOptions) (Queue, error) {
	...
	//db, err := sql.Open("libsql", fmt.Sprintf("file:%s?_journal_mode=experimental_mvcc&_busy_timeout=10000", opts.DBPath))
	db, err := sql.Open("libsql", fmt.Sprintf(
		"file:%s?_journal_mode=WAL&_txlock=immediate&_busy_timeout=10000",
		opts.DBPath,
	))
	if err != nil {
...
func (q *TursoQueue) createTables() error {
	schema := `
...
func (q *TursoQueue) Add(ctx context.Context, urlStr string) error {
...
func (q *TursoQueue) FetchNext(ctx context.Context) (*Request, error) {
...
func (q *TursoQueue) MarkHandled(req *Request) error {
	return q.MarkHandledWithError(req, nil)
}

func (q *TursoQueue) MarkHandledWithError(req *Request, handleErr error) error {
...
func (q *TursoQueue) IsEmpty() (bool, error) {
	var count int
	err := q.db.QueryRow(
...
func (q *TursoQueue) Close() error {
	return q.db.Close()
}

func (q *TursoQueue) GetStats() (map[string]int, error) {
...
func (q *TursoQueue) ResetStuckRequests(timeout time.Duration) error {
...
func (q *TursoQueue) GetAllHosts(ctx context.Context) ([]string, error) {
